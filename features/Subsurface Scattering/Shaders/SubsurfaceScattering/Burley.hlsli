...
BurleySampleDiffuseNormal SampleSSSColor(float2 CenterUV, float2 UVOffset, float MipLevel, uint eyeIndex)
{
	...
	Sample.DiffuseLighting.rgb = Color::IrradianceToLinear(Sample.DiffuseLighting.rgb / max(AlbedoTexture[pixCoord].rgb, EPSILON_SSS_ALBEDO));
...
float4 BurleyNormalizedSS(uint2 DTid, float2 texCoord, uint eyeIndex, float sssAmount, bool humanProfile)
{
	...
	OutColor.xyz = Color::IrradianceToGamma(OutColor.xyz) * AlbedoTexture[DTid.xy].rgb;
