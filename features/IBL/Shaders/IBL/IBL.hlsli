...
namespace ImageBasedLighting
{
	...
	float3 GetFogIBLColor(float3 fogColor)
	{
		...
		float3 iblColor = Color::GammaToLinear(directionalAmbientColor * SharedData::iblSettings.DALCAmount) + Color::Saturation(GetSkyDiffuseIBL(float3(0, 0, 0)), SharedData::iblSettings.IBLSaturation) * SharedData::iblSettings.DiffuseIBLScale;
