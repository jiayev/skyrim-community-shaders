...
{
	float red = (float)slice / (float)sliceCount;
	float green = 0.01;
	float blue = 1.0 - (float)slice / (float)sliceCount;
	...
	Texture2D<lpfloat> sourceViewspaceDepth, SamplerState depthSampler, RWTexture2D<uint> outWorkingAOTerm, RWTexture2D<unorm float> outWorkingEdges)
{
	...
	// see "Algorithm 1" in https://www.activision.com/cdn/research/Practical_Real_Time_Strategies_for_Accurate_Indirect_Occlusion_NEW%20VERSION_COLOR.pdf
	{
		...
		for (lpfloat slice = 0; slice < sliceCount; slice++) {
			...
			[unroll] for (lpfloat step = 0; step < stepsPerSlice; step++)
			{
...
uint4 GatherAlternative(const GTAOConstants consts, Texture2D<uint> sourceAOTerm, float2 gatherCenter, int2 offset)
{
	// Calculate texel offsets
	float2 texelSize = rcp(consts.ViewportPixelSize);
	float2 offset1 = float2(-texelSize.x, -texelSize.y);  // Top-left
	float2 offset2 = float2(texelSize.x, -texelSize.y);   // Top-right
	float2 offset3 = float2(-texelSize.x, texelSize.y);   // Bottom-left
	float2 offset4 = float2(texelSize.x, texelSize.y);    // Bottom-right

	// Manually sample texels using the [] operator
	float4 sample1 = sourceAOTerm[gatherCenter + offset1 + offset];  // Top-left texel
	float4 sample2 = sourceAOTerm[gatherCenter + offset2 + offset];  // Top-right texel
	float4 sample3 = sourceAOTerm[gatherCenter + offset3 + offset];  // Bottom-left texel
	float4 sample4 = sourceAOTerm[gatherCenter + offset4 + offset];  // Bottom-right texel

	// Gather the red channel values manually
	return uint4(sample1.r, sample2.r, sample3.r, sample4.r);
}
...
void XeGTAO_Denoise(const uint2 pixCoordBase, const GTAOConstants consts, Texture2D<uint> sourceAOTerm, Texture2D<lpfloat> sourceEdges, SamplerState texSampler, RWTexture2D<uint> outputTexture, const uniform bool finalApply)
{
	...
	AOTermType visQ0[4];
	XeGTAO_DecodeGatherPartial(GatherAlternative(consts, sourceAOTerm, gatherCenter, int2(0, 0)), visQ0);
	AOTermType visQ1[4];
	XeGTAO_DecodeGatherPartial(GatherAlternative(consts, sourceAOTerm, gatherCenter, int2(2, 0)), visQ1);
	AOTermType visQ2[4];
	XeGTAO_DecodeGatherPartial(GatherAlternative(consts, sourceAOTerm, gatherCenter, int2(0, 2)), visQ2);
	AOTermType visQ3[4];
	XeGTAO_DecodeGatherPartial(GatherAlternative(consts, sourceAOTerm, gatherCenter, int2(2, 2)), visQ3);
	...
	for (int side = 0; side < 2; side++) {
		...
		lpfloat edginess = (saturate(4.0 - leak_threshold - dot(edgesC_LRTB[side], 1)) / (4 - leak_threshold)) * leak_strength;
