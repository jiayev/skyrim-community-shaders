...
[numthreads(8, 8, 1)] void main(uint2 tid : SV_DispatchThreadID) {
	uint2 textureDim;
	RWTexOutput.GetDimensions(textureDim.x, textureDim.y);
	const uint2 frameDim = uint2(clamp(float2(textureDim) * FrameBuffer::DynamicResolutionParams1.xy, 1.0, float2(textureDim)));
