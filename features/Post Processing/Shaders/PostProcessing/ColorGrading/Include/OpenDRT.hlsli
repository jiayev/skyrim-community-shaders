...
float3 display_gamut_whitepoint(float3 rgb, float tsn, float creative_white_limit, int display_gamut, int creative_white) {
  ...
  if (creative_white==0) rgb = mul(matrix_cat_d65_to_d93, rgb);
  else if (creative_white==1) rgb = mul(matrix_cat_d65_to_d75, rgb);
  else if (creative_white==3) rgb = mul(matrix_cat_d65_to_d60, rgb);
  else if (creative_white==4) rgb = mul(matrix_cat_d65_to_d55, rgb);
  else if (creative_white==5) rgb = mul(matrix_cat_d65_to_d50, rgb);
  ...
  float cwp_norm = 1.0f;
  if (display_gamut == 0) {
    if (creative_white == 0) cwp_norm = 0.744192699063f;
    else if (creative_white == 1) cwp_norm = 0.873470832146f;
    else if (creative_white == 3) cwp_norm = 0.955936992163f;
    else if (creative_white == 4) cwp_norm = 0.905671332781f;
    else if (creative_white == 5) cwp_norm = 0.850004385027f;
  } 
  else if (display_gamut == 1) { // P3D65 or P3 Limited Rec.2020
    if (creative_white == 0) cwp_norm = 0.762687057298f; // D93
    else if (creative_white == 1) cwp_norm = 0.884054083328f; // D75
    // else if (cwp == 2) cwp_norm = 1.0f; // D65
    else if (creative_white == 3) cwp_norm = 0.964320186739f; // D60
    else if (creative_white == 4) cwp_norm = 0.923076518860f; // D55
    else if (creative_white == 5) cwp_norm = 0.876572837784f; // D50
  }
...
float3 OpenDRTTransform(float3 rgb)
{
  ...
  if (ptm_enable != 0) {
    ...
    else ptm_low_f = 1.0f + ptm_low*exp(-2.0f*ach_d*ach_d/ptm_low_st)*pow(1.0f - tsn_const, 1.0f/ptm_low_rng);
    float ptm_high_f;
    if (ptm_high_st == 0.0f || ptm_high_rng == 0.0f) ptm_high_f = 1.0f;
    else ptm_high_f = 1.0f + ptm_high*exp(-2.0f*ach_d*ach_d/ptm_high_st)*pow(tsn_pt, 1.0f/(4.0f*ptm_high_rng));
  ...
  // Purity Compress Low
  if (ptl_enable != 0) rgb = float3(softplus(rgb.x, ptl_c), softplus(rgb.y, ptl_m), softplus(rgb.z, ptl_y));
  ...
  // Clamp
  if (clamp != 0) rgb = clampf3(rgb, 0.0f, 1.0f);
