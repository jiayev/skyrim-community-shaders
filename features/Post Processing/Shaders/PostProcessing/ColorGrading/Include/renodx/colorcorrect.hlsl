...
#undef GAMMA_SAFE
			...
			{
				if (method == 1)
					return ChrominanceICtCp(incorrect_color, correct_color, strength, clamp_chrominance_loss);
				if (method == 2)
					return ChrominancedtUCS(incorrect_color, correct_color, strength, clamp_chrominance_loss);
