...
namespace sk_hdr_png
{
	...
	{
		r16g16b16a16_float,     // scRGB / scene-linear BT.709 (ReShade back buffer)
		r16g16b16a16_pq,        // BT.2020 PQ already applied (Community Shaders HDROutputCS)
		r10g10b10a2_unorm,
		b10g10r10a2_unorm,
	};
...
#define DeclareUint32(x,y) uint32_t x = SetUint32((x),(y))
  ...
  static auto LinearToPQ = [](DirectX::FXMVECTOR N)
  {
  ...
  static auto PQToLinear = [](DirectX::FXMVECTOR N)
  {
    using namespace DirectX;
...
sk_hdr_png::write_image_to_disk (const wchar_t* image_path, unsigned int width, unsigned int height, const void* pixels, int quantization_bits, format fmt, bool use_clipboard)//, reshade::api::display* display)
{
	...
	if (image_path == nullptr || width == 0 || height == 0 || !validFormat)
	{
		return false;
	}
	...
	{
		hr =
			CoCreateInstance(CLSID_WICImagingFactory, NULL, CLSCTX_INPROC_SERVER, IID_IWICImagingFactory, factory.put_void());
		...
		if (SUCCEEDED(hr)) hr = factory->CreateEncoder(GUID_ContainerFormatPng, NULL, encoder.put());
		if (SUCCEEDED(hr)) hr = encoder->Initialize(stream.get(), WICBitmapEncoderNoCache);
		if (SUCCEEDED(hr)) hr = encoder->CreateNewFrame(bitmap_frame.put(), property_bag.put());
		...
		if (SUCCEEDED(hr))
		{
			...
			else if (fmt == format::r16g16b16a16_pq)
			{
				...
				for (size_t y = 0; y < height; y++)
				{
					...
					for (size_t x = 0; x < width; x++)
					{
						XMVECTOR rgb = XMVectorSaturate(XMLoadFloat4(rgba32_pixels++));

						*pixel_luminance++ =
							XMVectorGetY(
								XMVector3Transform(
									PQToLinear(rgb), c_from2020toXYZ));
