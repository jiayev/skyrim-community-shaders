...
#	if defined(DEFERRED) && defined(CLOUDS) && defined(CLOUD_SHADOWS) && defined(PHYSICAL_SKY)
	...
	if (brightness > 0.0 && baseColor.w > 0.0)
	{
		...
		if (SharedData::physSkyData.enabled) {
			cloudColor = baseColor.a * baseColor.xyz * brightness * (1.0 - saturate(dot(rayShadow, 0.25))) * SharedData::DirLightColor.xyz;
			cloudColor *= PhysSky::SampleTr(normalize(SharedData::DirLightDirection.xyz), SampBaseSampler);
		}
