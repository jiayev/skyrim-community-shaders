...
float3 GetLightSpecularInputHair(PS_INPUT input, float3 L, float3 V, float3 N, float3 lightColor, float shininess, float2 uv, float3 baseColor)
{
	...
	const float3 specSecondary = Hair::D_KajiyaKay(TshiftSecondary, H, shininess * 0.5) * 0.3;
...
#		elif !defined(FACEGEN) && !defined(MULTI_LAYER_PARALLAX) && !defined(PARALLAX) && !defined(SPARKLE)
	if (ProjectedUVParams3.w > 0.5) {
		...
		if (!SharedData::linearLightingSettings.enableLinearLighting) {
			...
			projBaseColor = Color::Diffuse(TexProjDiffuseSampler.Sample(SampProjDiffuseSampler, projNormalDiffuseUv).xyz) * Color::Diffuse(ProjectedUVParams2.xyz);
		}
...
#			if defined(CS_SKIN)
	if (SharedData::skinData.skinParams2.y) {
		rainWetness += SharedData::skinData.skinParams2.y;
		puddleWetness += SharedData::skinData.skinParams2.y;
	}
	if (SharedData::skinData.ApplySpecularToWetness) {
...
#		if defined(CS_HAIR)
	if (SharedData::hairSpecularSettings.Enabled)
		vertexColor = 1;
	float3 indirectSpecularLobeWeight = Hair::GetHairIndirectSpecularLobeWeights(worldSpaceNormal.xyz, worldSpaceViewDirection, worldSpaceVertexNormal, SharedData::hairSpecularSettings.Glossiness);
#		endif  // CS_HAIR
...
#		endif
	if (FrameBuffer::FrameParams.y && FrameBuffer::FrameParams.z)
		if (!SharedData::linearLightingSettings.enableLinearLighting) {
			...
			color.xyz = lerp(color.xyz, Color::GammaToTrueLinear(input.FogParam.xyz), Color::GammaToLinear(input.FogParam.w));
		}
#	endif
