...
#		endif  // MODELSPACENORMALS
		...
		if (skinWetness > 0.0f) {
			...
			modelNormal.xyz = wetModelNormal;
		}
	}
#	endif  // CS_SKIN
...
#		if !defined(DRAW_IN_WORLDSPACE)
			bentNormal = GTAO::GetModelSpaceBentNormal(screenUV.xy, eyeIndex, xeGTAOWeight, bentNormalWS, input.WorldSpace, input.World[eyeIndex]);
#		else
			bentNormal = GTAO::GetModelSpaceBentNormal(screenUV.xy, eyeIndex, xeGTAOWeight, bentNormalWS);
#		endif
...
#		if defined(SPECULAR) || defined(SPARKLE)
		lightsSpecularColor = GetLightSpecularInput(input, DirLightDirection, viewDirection, modelNormal.xyz, dirLightColor.xyz * dirDetailShadow, F0, shininess, uv);
#		endif
...
#				if defined(SPECULAR) || (defined(SPARKLE) && !defined(SNOW))
			lightsSpecularColor += GetLightSpecularInput(input, normalizedLightDirection, viewDirection, modelNormal.xyz, lightColor, F0, shininess, uv);
...
#				if defined(SPECULAR) || (defined(SPARKLE) && !defined(SNOW))
			lightsSpecularColor += GetLightSpecularInput(input, normalizedLightDirection, worldSpaceViewDirection, worldSpaceNormal.xyz, lightColor, F0, shininess, uv);
#				endif
...
#		endif
	...
	float3 wetnessReflectance = WetnessEffects::GetWetnessAmbientSpecular(screenUV, wetnessNormal, worldSpaceVertexNormal, worldSpaceViewDirection, waterRoughnessSpecular);
...
#		if defined(CS_HAIR)
	...
	if (SharedData::hairSpecularSettings.Enabled)
		vertexColor = 1;
	Hair::GetHairIndirectSpecularLobeWeights(indirectDiffuseLobeWeight, indirectSpecularLobeWeightPrim, indirectSpecularLobeWeightSec, hairT, worldSpaceNormal.xyz, worldSpaceViewDirection, worldSpaceVertexNormal, SharedData::hairSpecularSettings.HairGlossiness, uv, baseColor.xyz);
...
#			if defined(XeGTAO)
	if (SharedData::xeGTAOSettings.Enabled && SharedData::xeGTAOSettings.BentNormals) {
		specularOcclusion = BentNormals::SpecularAO_Cones(bentNormalWS, worldSpaceNormal, worldSpaceViewDirection, xeGTAOWeight, pbrSurfaceProperties.Roughness);
...
#				if defined(SKYLIGHTING)
	{
		float3 indirectSpecular = Hair::GetHairDynamicCubemapSpecularIrradiance(uv, screenUV, hairT, worldSpaceNormal, worldSpaceVertexNormal, worldSpaceViewDirection, SharedData::hairSpecularSettings.HairGlossiness, indirectSpecularLobeWeightPrim, indirectSpecularLobeWeightSec, skylightingSH);
		color.xyz += indirectSpecular;
	}
#				else
	{
		float3 indirectSpecular = Hair::GetHairDynamicCubemapSpecularIrradiance(uv, screenUV, hairT, worldSpaceNormal, worldSpaceVertexNormal, worldSpaceViewDirection, SharedData::hairSpecularSettings.HairGlossiness, indirectSpecularLobeWeightPrim, indirectSpecularLobeWeightSec);
		color.xyz += indirectSpecular;
	}
#				endif
#			endif
...
#	if defined(SPECULAR) && !defined(VANILLA_FRESNEL_DL) && !(defined(SKIN) && defined(CS_SKIN))
#		if defined(EMAT_ENVMAP)
...
#		if defined(HAIR) && defined(CS_HAIR)
	if (SharedData::hairSpecularSettings.Enabled) {
		roughness = pow(abs(2.0 / (glossiness * 0.5 + 2.0)), 0.25);
	}
#		endif
...
#			if defined(WETNESS_EFFECTS)
		...
		psout.NormalGlossiness = float4(GBuffer::EncodeNormal(screenSpaceNormal), lerp(1.0 - roughness, saturate(1.0 - roughness + wetnessGlossinessGain), wetnessGlossinessSpecular), psout.Diffuse.w);
#			else
...
#	endif
	...
	if (!inWorld && SharedData::linearLightingSettings.enableLinearLighting && !(Permutation::PixelShaderDescriptor & Permutation::LightingFlags::DefShadow)) {
