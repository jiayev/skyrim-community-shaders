...
// Relative-colorimetric conversion for grading. XYZ denotes XYZ D65 here;
// getRGBMatrix remains available for unadapted RGB/XYZ basis changes.
inline DirectX::SimpleMath::Matrix getWhiteAdaptedRGBMatrix(std::string_view in_space, std::string_view out_space)
{
	...
	// Bradford, matching the shared shader sRGB/AP1 conversions.
	const Matrix cone{ DirectX::XMFLOAT3X3{
	...
	const Matrix adaptation = cone.Invert() * scale * cone;
