...
namespace ACES2
{
	...
	ACES2CB ComputeParams(float peakLuminance, bool hdr)
	{
		...
		JMhParamsCPU limit_params = init_JMhParams(hdr ? P3D65_CHROM : SRGB_CHROM);
		...
		if (hdr) {
			// P3-D65 -> Rec.2020
			Mat3 P3D65_to_XYZ = RGBtoXYZ(P3D65_CHROM, 1.0f);
			Mat3 XYZ_to_Rec2020 = XYZtoRGB(REC2020_CHROM, 1.0f);
			Mat3 P3D65_to_Rec2020 = mul_mm(P3D65_to_XYZ, XYZ_to_Rec2020);
