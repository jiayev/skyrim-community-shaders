...
namespace
{
	...
	void DrawAllRGBSliders(const AllRGBControl& control, float& allValue, float* rgbValues, OnAllChanged onAllChanged)
	{
		...
		Util::RGBFloatDrag3("##RGB", rgbValues, AllRGBDragSpeed, control.min, control.max, control.format);
		ImGui::SameLine();
...
void ColorGrading::DrawSettings()
{
	...
	ImGui::SeparatorText("Color Grading");
	{
		...
		if (ImGui::TreeNode("ASC CDL")) {
			DrawRGBAllControls(settings, kAscCdlControls);
			ImGui::TreePop();
		}
		...
		if (ImGui::TreeNode("Shadows/Midtones/Highlights")) {
			DrawRGBAllControls(settings, kShadowsMidtonesHighlightsControls);
		...
		if (ImGui::TreeNode("Contrast")) {
			DrawRGBAllControls(settings, kContrastControls);
			ImGui::TreePop();
		}

		ImGui::Text("Post-Tonemapping Settings");
		if (ImGui::TreeNode("Lift Gamma Gain")) {
			DrawStoredAllRGBControls(settings, kLiftGammaGainControls);
			ImGui::TreePop();
		}
	}
