...
void WetnessEffects::DrawSettings()
{
	...
	if (ImGui::TreeNodeEx(T(TKEY("raindrop_effects"), "Raindrop Effects"), ImGuiTreeNodeFlags_DefaultOpen)) {
		...
		if (ImGui::TreeNodeEx(T(TKEY("wall_rain_flow"), "Wall Rain Flow"))) {
			ImGui::SliderFloat(T(TKEY("wall_rain_density"), "Density"), &settings.WallRainDensity, 0.5f, 3.0f, "%.2f");
			if (auto _tt = Util::HoverTooltipWrapper())
				ImGui::Text("%s", T(TKEY("wall_rain_density_tooltip"), "Overall wall-rain coverage. Higher values add more streaks, especially in heavier rain."));

			ImGui::SliderFloat(T(TKEY("wall_rain_drop_size"), "Drop Size"), &settings.WallRainDropSize, 0.5f, 2.0f, "%.2f");
			if (auto _tt = Util::HoverTooltipWrapper())
				ImGui::Text("%s", T(TKEY("wall_rain_drop_size_tooltip"), "Size of individual wall streak droplets. Higher values make drops larger."));

			ImGui::SliderFloat(T(TKEY("wall_rain_flow_speed"), "Flow Speed"), &settings.WallRainSpeed, 0.2f, 2.0f, "%.2f");
			if (auto _tt = Util::HoverTooltipWrapper())
				ImGui::Text("%s", T(TKEY("wall_rain_flow_speed_tooltip"), "How fast wall rain moves downward."));

			ImGui::SliderFloat(T(TKEY("wall_rain_layering"), "Layering"), &settings.WallRainLayering, 0.0f, 2.0f, "%.2f");
			if (auto _tt = Util::HoverTooltipWrapper())
				ImGui::Text("%s", T(TKEY("wall_rain_layering_tooltip"), "Strength of the secondary streak layer that fills gaps during heavier rain."));

			ImGui::SliderFloat(T(TKEY("wall_rain_normal_strength"), "Normal Strength"), &settings.WallRainNormalStrength, 0.0f, 4.0f, "%.2f");
			if (auto _tt = Util::HoverTooltipWrapper())
				ImGui::Text("%s", T(TKEY("wall_rain_normal_strength_tooltip"), "How strongly wall rain distorts surface normals."));

			ImGui::SliderFloat(T(TKEY("wall_rain_wet_look"), "Wet Look"), &settings.WallRainWetness, 0.0f, 2.0f, "%.2f");
			if (auto _tt = Util::HoverTooltipWrapper())
				ImGui::Text("%s", T(TKEY("wall_rain_wet_look_tooltip"), "Boosts the glossy wet highlight created by wall rain."));

			ImGui::SliderFloat(T(TKEY("wall_rain_darkening"), "Darkening"), &settings.WallRainDarkening, 0.0f, 1.5f, "%.2f");
			if (auto _tt = Util::HoverTooltipWrapper())
				ImGui::Text("%s", T(TKEY("wall_rain_darkening_tooltip"), "How much wall rain darkens the underlying surface."));
	...
	if (weatherEditor.loaded) {
		auto weName = weatherEditor.GetName();
		if (ImGui::SmallButton(std::vformat(T(TKEY("open_feature"), "Open {}"), std::make_format_args(weName)).c_str())) {
			// Navigate to the replacement feature in the menu
			Menu::GetSingleton()->SelectFeatureMenu(weatherEditor.GetShortName());
		}
		if (auto _tt = Util::HoverTooltipWrapper()) {
			ImGui::Text(T(TKEY("open_installed_feature_tooltip"), "Open the installed %s feature"), weatherEditor.GetName().c_str());
		}
	}
