...
void PhysicalSky::Reset()
{
	...
	float2 dynres = Util::ConvertToDynamic(res, true);
...
void PhysicalSky::Prepass()
{
	if (cbData.enabled) {
		...
		if (!globals::features::raytracing.IsPathTracing())
			RenderView(Util::GetCurrentSceneDepthSRV());

		// Volumetric clouds
		if (!renderVolumetricClouds && texVolTr && texVolLum) {
...
void PhysicalSky::RenderView(ID3D11ShaderResourceView* depth)
{
	if (!cbData.enabled || !depth)
		return;
	auto* context = globals::d3d::context;
	ID3D11ShaderResourceView* shadow = nullptr;
	context->PSSetShaderResources(64, 1, &shadow);
	context->CSSetShaderResources(64, 1, &shadow);
	AccumShadow(depth);
	shadow = texApShadow->srv.get();
	context->PSSetShaderResources(64, 1, &shadow);
	if (settings.enableVolumetricClouds && !globals::deferred->MediumCompositeEnabled())
		RenderVolumetricClouds(VolumetricCloudPass::kMainViewAndCubemap, depth);
}
...
void PhysicalSky::AccumShadow(ID3D11ShaderResourceView* depth)
{
	auto state = globals::state;
	auto context = globals::d3d::context;
	...
	if (!volumetricShadows.loaded) {
		const float clear[4]{};
		context->ClearUnorderedAccessViewFloat(texApShadow->uav.get(), clear);
		return;
	}
	...
	uint resolution[2] = {
		static_cast<uint>(std::clamp(texApShadow->desc.Width * scale.x, 1.f, float(texApShadow->desc.Width))),
		static_cast<uint>(std::clamp(texApShadow->desc.Height * scale.y, 1.f, float(texApShadow->desc.Height)))
	};
...
void PhysicalSky::BindCompositeResources()
{
	auto* context = globals::d3d::context;
	ID3D11ShaderResourceView* atmosphere[] = {
		texTrLut ? texTrLut->srv.get() : nullptr,
		texSvLut ? texSvLut->srv.get() : nullptr,
		texApLut ? texApLut->srv.get() : nullptr,
		texApShadow ? texApShadow->srv.get() : nullptr
	};
	ID3D11ShaderResourceView* clouds[] = {
		texVolFilteredTr ? texVolFilteredTr->srv.get() : nullptr,
		texVolFilteredLum ? texVolFilteredLum->srv.get() : nullptr,
		texShadowVolume ? texShadowVolume->srv.get() : nullptr,
		texApSunLut ? texApSunLut->srv.get() : nullptr,
		texVolCubeTr ? texVolCubeTr->srv.get() : nullptr,
		texVolCubeLum ? texVolCubeLum->srv.get() : nullptr
	};
	context->CSSetShaderResources(61, ARRAYSIZE(atmosphere), atmosphere);
	context->CSSetShaderResources(110, ARRAYSIZE(clouds), clouds);
	auto* sampler = sampSv.get();
	context->CSSetSamplers(1, 1, &sampler);
}
